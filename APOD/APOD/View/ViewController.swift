...
// MARK: - View
/// View는 사용자 입력을 받아 -> `Action`으로 변경 -> `Reactor`에 전달
/// `Reactor`로부터 전달받은 상태를 렌더링
///
/// `View`와 `ViewController`를 같이 하나의 `View`로 취급
/// Business Logic을 담당하지 않음, `오직 Rendering에만 집중`
class ViewController: UIViewController, WKNavigationDelegate, ReactorKit.View {
...
//        bindUI()
    ...
    func bind(reactor: ViewReactor) {
        ...
        /// `observe(on:)`: Data Sequence를 `어떤 Scheduler에서 observe할 것인지`를 결정
        /// (-> 즉, Observer가 Observable을 관찰할 Scheduler를 지정)
    ...
    @objc private func clearButtonPressed() -> Void {
        ...
        DispatchQueue.main.async {
            ...
            if (self.viewModel.media.value == nil) {
                self.apodImageView.image = nil
                self.apodWebView.isHidden = true
            }
